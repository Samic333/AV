  ...
  return (
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-navy-900 mb-2">Earnings</h1>
          <p className="text-navy-600">View your earnings and request payouts</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card>
            <p className="text-sm text-navy-600 mb-1">Total Earnings</p>
            <p className="text-3xl font-bold text-navy-900">
              {isLoading ? '...' : `$${(wallet?.totalEarned || 0).toFixed(2)}`}
            </p>
          </Card>
          <Card>
            <p className="text-sm text-navy-600 mb-1">Available Balance</p>
            <p className="text-3xl font-bold text-sky-blue-600">
              {isLoading ? '...' : `$${(wallet?.balance || 0).toFixed(2)}`}
            </p>
          </Card>
          <Card>
            <p className="text-sm text-navy-600 mb-1">Pending Payout</p>
            <p className="text-3xl font-bold text-navy-900">
              {isLoading ? '...' : `$${(wallet?.pendingBalance || 0).toFixed(2)}`}
            </p>
          </Card>
          <Card>
            <p className="text-sm text-navy-600 mb-1">This Month</p>
            <p className="text-3xl font-bold text-navy-900">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <Card>
              <h2 className="text-xl font-semibold text-navy-900 mb-4">Earnings History</h2>
              {isLoading ? (
                ...
                <div className="text-center py-12">
                  <p className="text-navy-600 mb-4">No earnings yet</p>
                  <p className="text-sm text-navy-500">Start teaching to earn money!</p>
                </div>
              ) : (
                    ...
                    <div key={earning.id} className="border-b border-gray-200 pb-4 last:border-0">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-navy-500">After platform commission</p>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div>
            <Card>
              <h2 className="text-xl font-semibold text-navy-900 mb-4">Request Payout</h2>
              <div className="mb-4">
                <p className="text-sm text-navy-600 mb-2">Available Balance</p>
                <p className="text-3xl font-bold text-sky-blue-600">
                  {isLoading ? '...' : `$${(wallet?.balance || 0).toFixed(2)}`}
                </p>
              </div>
